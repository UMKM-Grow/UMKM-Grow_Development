...
const verifyPromo = async (req, res) => {
  try {
    ...
    if (promo.branch_id && promo.branch_id !== branchId) {
      return res
        .status(400)
        .json({ message: "Kode promo tidak berlaku di cabang ini!" });
    }

    if (
      total < Number(promo.minimal_belanja) ||
      Number(promo.minimal_belanja) < 0
    ) {
      return res
        .status(400)
        .json({
          message: `Minimal belanja Rp ${promo.minimal_belanja} belum terpenuhi.`,
        });
    }
    ...
    total_diskon = Number.isFinite(total_diskon)
      ? Math.min(total_diskon, total)
      : 0;
